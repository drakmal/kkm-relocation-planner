...
## Facility data (Supabase `locations`, ~3,400 rows)
...
0a. **⏰ [REMINDER for owner] Remove all emojis from the UI** — owner doesn't want any. Known spots: `app/report/[id]/page.tsx:110` (🏠 🏢 + the → arrow), `app/page.tsx:632` (📍), `app/page.tsx:719` (📅). Replace with plain text/labels (e.g. "Leave home … → Arrive office …" without icons). Telegram message emoji (🔔 in `notify_feedback.py`) is not web UI — leave unless owner says otherwise. **Don't add new emojis to UI going forward.**
...
6. ⚠️ **[Bug FIXED — needs re-test] Empty reports root cause.** The two test requests produced empty reports ("insufficient data", 0 logs) NOT because of the LLM — every Google Distance Matrix call failed with `INVALID_REQUEST: departure_time is in the past`. `collect_traffic.py` captured `now` once at the start, but a 100+ anchor batch takes minutes, so by call time the leave-home moment was past → Google rejects it. **Fixed:** `get_google_maps_travel_time` now clamps `departure_time` to `now+1min` if it's past. Also: report page + Groq prompt now include the user's leave-home/arrive-office times. **TO RE-TEST:** submit a fresh request for a future weekday (the 2 old requests' single-day window already passed) and confirm logs collect + a real report generates.
